%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   AUTHORS:                        % DATE:17/01/2018
%   - Ferran Carrasquer             % TEAM:5
%   - Juan Felipe Montesinos        % UNI:UPF 
%   - Yi Xiao
...
p1_2 = [points_1(1:2, matches1_2(1,:)); ones(1, length(matches1_2))];
p2_1 = [points_2(1:2, matches1_2(2,:)); ones(1, length(matches1_2))];
[F1_2, inliers1_2] = ransac_fundamental_matrix(p1_2, p2_1, 2.0); 

% Between im1 and im3:
matches1_3 = siftmatch(desc_1, desc_3);
p1_3 = [points_1(1:2, matches1_3(1,:)); ones(1, length(matches1_3))];
p3_1 = [points_3(1:2, matches1_3(2,:)); ones(1, length(matches1_3))];
[F1_3, inliers1_3] = ransac_fundamental_matrix(p1_3, p3_1, 2.0); 

% Between im1 and im4:
matches1_4 = siftmatch(desc_1, desc_4);
p1_4 = [points_1(1:2, matches1_4(1,:)); ones(1, length(matches1_4))];
p4_1 = [points_4(1:2, matches1_4(2,:)); ones(1, length(matches1_4))];
[F1_4, inliers1_4] = ransac_fundamental_matrix(p1_4, p4_1, 2.0); 
...
idx_car_I1 = 1197;
% ToDo: identify the corresponding point of idx_car_I1 in image 2
match= find(matches1_2(1,:)==idx_car_I1);
idx_car_I2 = matches1_2(2,match);
% ToDo: identify the corresponding point of idx_car_I1 in image 3
match= find(matches1_3(1,:)==idx_car_I1);
idx_car_I3 = matches1_3(2,match);
% ToDo: identify the corresponding point of idx_car_I1 in image 4
match= find(matches1_4(1,:)==idx_car_I1);
idx_car_I4 = matches1_4(2,match);
