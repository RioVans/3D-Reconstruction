...
addpath('Data'); % add the path of datasets
...
for i=1:4
    ...
    projection_1=P1*X;
    projection_2=P2*X;
    if (projection_1(3)>=0) && (projection_2(3)>=0)
        P2=Pc2{i};
        break;
    end
end
...
for i = 1:length(X)
    d1(1,i)=sqrt((x1(1,i)-proj_x1(1,i)).^2+(x1(2,i)-proj_x1(2,i)).^2);
    d2(1,i)=sqrt((x2(1,i)-proj_x2(1,i)).^2+(x2(2,i)-proj_x2(2,i)).^2);
end
error=d1.^2+d2.^2;
figure;hist(error,50);
hold on;
line([mean(error), mean(error)], [0,400], 'LineWidth', 2, 'Color', 'black');
text(mean(error),420,'mean reprojection error');
...
im1=imread('scene1.row3.col4.ppm');
im2=imread('scene1.row3.col3.ppm');
disparityMap_SSD = stereo_computation(im1, im2, 0, 16, 9, 'SSD');
figure;imshow(disparityMap_SSD);
...
im1=imread('scene1.row3.col4.ppm');
im2=imread('scene1.row3.col3.ppm');
disparityMap_NCC = stereo_computation(im1, im2, 0, 16, 9, 'NCC');
figure;imshow(disparityMap_NCC);
...
% change.
im1=imread('0001_rectified_s.png');
im2=imread('0002_rectified_s.png');
disparityMap_SSD_test = stereo_computation(im1, im2, -50, 20, 9, 'SSD');
figure;imshow(disparityMap_SSD_test);
disparityMap_NCC_test = stereo_computation(im1, im2, -50, 20, 9, 'NCC');
figure;imshow(disparityMap_NCC_test);
...
im1=imread('scene1.row3.col4.ppm');
im2=imread('scene1.row3.col3.ppm');
disparityMap_BW = stereo_computation(im1, im2, 0, 16, 35, 'BW');
figure;imshow(disparityMap_BW);
